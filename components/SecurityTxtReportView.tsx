...
export function SecurityTxtReportView({ report }: { report: SecurityTxtReport }) {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-ink-700 bg-ink-900/60 p-5">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="min-w-0">
            <p className="text-xs uppercase tracking-wider text-slate-500">Audited policy</p>
            ...
            <p className="text-xs text-slate-500 mt-1">
              ...
              {target.redirects && target.redirects.length > 0
                ? ` · ${target.redirects.length} redirect(s)`
                : ""}
            </p>
          </div>
          <div className="flex items-center gap-6">
            ...
            <div className="grid grid-cols-4 gap-2 text-center">
              ...
              <div className="rounded-lg border border-sky-500/30 bg-sky-500/5 px-3 py-2">
                <p className="text-sky-300 text-lg font-semibold">{summary.info}</p>
                <p className="text-[10px] uppercase tracking-wider text-slate-500">Info</p>
              </div>
            </div>
          </div>
        </div>
      ...
      {groups.map((group) => (
        <section key={group.id} className="rounded-2xl border border-ink-700 bg-ink-900/40 p-5">
